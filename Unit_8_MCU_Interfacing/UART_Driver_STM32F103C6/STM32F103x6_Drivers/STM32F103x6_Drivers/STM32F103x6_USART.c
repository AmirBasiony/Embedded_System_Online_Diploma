 ...
 * @param [in]   -USARTx : pointer to UART_Config Structure that Contains
...
void MCAL_USART_Handling_Init(USART_TypeDef * USARTx,uint32_t BaudRate , uint8_t HwFlowCtl,uint8_t Parity_Mode,uint8_t IQR_Enable ,uint8_t Payload_Length,uint8_t StopBits,uint8_t USART_Mode,void (*Ptr_IRQ_Handler)(void))
{
	...
	UART_CFG.HwFlowCtl= HwFlowCtl;
	UART_CFG.Parity_Mode= Parity_Mode;
	UART_CFG.IQR_Enable = IQR_Enable;
	UART_CFG.Payload_Length =Payload_Length;
	UART_CFG.StopBits = StopBits;
	UART_CFG.USART_Mode = USART_Mode;
	UART_CFG.P_IRQ_Handler = Ptr_IRQ_Handler;
